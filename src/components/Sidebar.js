...
function Navbar(props) {
    ...
    search.forEach((lst) => {
        ...
        login_result.push(
            <div className='lst_wrapper' style={{"display":"flex","justifyContent":"space-between","alignItems":"center"}}>
            <div className='lst_addr' id={repAddr} onClick={()=>{
                search_result_click(repAddr);
                if (props.pointObj.polyline != null){
                    props.setPointObj({
                        startPoint: { lat: props.userLat, lng: props.userLng },
                        endPoint: { lat: lst.lat, lng: lst.lng },
                        polyline: props.pointObj.polyline.setMap(null)
                    })
                } else{
                    props.setPointObj({
                        startPoint: { lat: props.userLat, lng: props.userLng },
                ...
                }} style={{"color":"white","marginLeft":"20px"}}>{lst.addr}</div>
            <button className='lst_btn' id={btnAddr} onClick={()=>{props.setFindRoute(true)}} style={{"width":"30px","height":"30px","background":"none","border":"0","color":"red","display":"none","cursor":"pointer"}}>
    ...
    const open_sidebar = () => {
        ...
        if(window.sessionStorage.userId != null){
            document.querySelector(".sidebar_myaccount").style.display = "block";
        }
        } else{
        ...
        document.querySelector(".sidebar_mysearch").style.display = "none";
        if(window.sessionStorage.userId != null){
            document.querySelector(".sidebar_myaccount").style.display = "none";
        }
        }
    }
    const visit_charge = async() => {
        if (window.confirm("방문충전 신청하시겠습니까?")){
            const url = `http://10.10.21.64:8080/api/request/${window.sessionStorage.userId}?lat=${props.userLat}&lng=${props.userLng}`
            const ajax = await fetch(url,{method:"Post"}); 
    ...
    <nav className="main-menu">
        <ul>
                ...
                <div className='sidebar_mysearch' style={{"height":"0"}}>
                    <div className='search_result'>
                        <div className='search_icon'>
                            <input id='find_addr' placeholder='주소검색'/>
            ...
            <div className='close_icon' style={{"display":"none"}} onClick={open_sidebar}>
    ...
    return(
        ...
        <nav className='sidebar_menu_btn'>
