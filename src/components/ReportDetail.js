...
function ReportDetail(){
    ...
    const [brdata,setBrdata] = useState([]);
        ...
        const 참고 = () => {
            const url = `http://10.10.21.64:8080/api/request?postNumber=userId=${window.sessionStorage.userId}`
        ...
        if (type === "request"){
            ...
            dataList.push(<div key={data.postNumber}>
                            ...
                            <td width="300"><button className="request_btn" onClick={()=>requestClick()}>OK</button></td>
                          </div>)
            });
            setContent(dataList);
        } else if (type === "report"){
            ...
            datalist2.push(<div key={data.postNum}>
                            ...
                            <td width="300">{data.postStartDate}</td>
                            <td width="300">{data.postEndDate}</td>
                            <td width="300"><button className="report_btn" onClick={()=>reportClick()}>break</button></td>
                           </div>)
            })
